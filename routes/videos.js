...
router.get('/q=:query', async (req, res) => {
    ...
    const apiVideos = await fetch(
        `https://www.googleapis.com/youtube/v3/search?part=snippet&order=viewCount&q=${query}&type=video&videoDefinition=high&key=AIzaSyBqP72gnX5iMsd_7Ij5CGND5OkLGuvY120&resultsPerPage=10&videoEmbeddable=true&maxResults=18`
...
router.get('/favorites', async (req, res) => {
    ...
    const detailsArray = await Promise.all(videoList.map(async videoDetails => {
        const details = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&order=viewCount&q=${videoDetails.video_reference}&type=video&videoDefinition=high&key=AIzaSyBqP72gnX5iMsd_7Ij5CGND5OkLGuvY120&resultsPerPage=10&videoEmbeddable=true&maxResults=10`).then((response) => response.json());
...
router.get('/t=:tag_name', async (req, res) => {
    ...
    const detailsArray = await Promise.all(videoList.map(async videoDetails => {
        const details = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&order=viewCount&q=${videoDetails}&type=video&videoDefinition=high&key=AIzaSyBqP72gnX5iMsd_7Ij5CGND5OkLGuvY120&resultsPerPage=10&videoEmbeddable=true&maxResults=10`).then((response) => response.json());
        return details.items[0]
    }));
